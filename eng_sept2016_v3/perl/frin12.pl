...
sub parsedrug {
	...
	if ($rxnstring) {
	    ...
	    $rxnstring =~ /\s*$rerxn\s*/;
	    my $rxn = $1;
	    if ($name eq 'CALCIUM') { $rxn = 1895; }
	    if ($name eq 'IRON') { $rxn = 90176; }
	    if ($name eq 'INSULIN') { $rxn = 631657; }
	    if ($name eq 'IVABRADINE') { $rxn = 1649480; }
	    if ($name eq 'ZINC') { $rxn = 11416; }
	    if ($name eq 'PIRFENIDONE') { $rxn = 1592254; }
	    if ($name eq 'ENZALUTAMIDE') { $rxn = 1307298; }
	    if ($name eq 'TRABECTEDIN') { $rxn = 1716278; }
	    print "rxNorm: $rxn\n";
